...
data class Event(
    val title: String,
    val description: String? = null,
    val daypart: Daypart,  // 类型从 String 改为 Daypart 枚举
    val durationMinutes: Int
)
```
...
// 遍历分组并输出结果
eventsByDaypart.forEach { (daypart, eventsList) ->
    println("${daypart.name}: ${eventsList.size} events")
}
```
...
// 扩展属性：为 Event 类添加 durationOfEvent 属性，不修改原类
