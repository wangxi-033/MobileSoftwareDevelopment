...
data class Event(
    val title: String,
    val description: String? = null,
    val daypart: Daypart, // 类型从String改为Daypart枚举
    val duration: Int
)
```
...
// 添加更多新活动的方法：调用列表 .add() 方法
eventList.add(Event(title = "New Activity", daypart = Daypart.EVENING, durationInMinutes = 20))
```
...
fun main() {
    ...
    groupByDaypart.forEach { (daypart, eventList) ->
        println("${daypart.name}: ${eventList.size} events")
    }
}
```
...
val Event.durationOfEvent: String
    get() {
        return if (this.durationInMinutes < 60) {
            "short"
        } else {
            "long"
        }
